...
(function($){
    ...
    $.fn.customSelect = function(options) {
        ...
        this.each(function(){
            ...
            if(sel.attr('disabled'))
                sel.wrap("<div class='custom-select-wrap custom-select-wrap_disabled'></div>");
            else
                sel.wrap("<div class='custom-select-wrap'></div>");
            ...
            // условие для подстановки другого текста для мобильной ерсии
            var selectedOption = (sel.find("option").eq(0)),
                selectedOptionVisible = selectedOption.css('display'),
                startText;

            if(selectedOptionVisible == 'none') { // тогда берем второй option
                startText = sel.find('option').eq(1).text();
            } else {
                startText = sel.find('option').eq(0).text();
            }
    ...
    $.fn.sinhronize  = function() {
        var self = $(this);

        self.prev(".custom-select-text").text(self.find("option:selected").text());
    };
...
;(function ($) {
  ...
  var pluginName = 'selectric',
    ...
    function _init(opts) {
      ...
      _this.classes = {};
      _this.element = element, _utils.triggerCallback('BeforeInit', _this);
    ...
    function _populate() {
      ...
      if (optionsLength = $options.length) {
        ...
        $options.each(function (i) {
          ...
          //if($elm.attr('data-text-mobile') && document.documentElement.clientWidth < 1000 ) _this.items[i].text = $elm.attr('data-text-mobile');
    ...
    function _detectItemVisibility(index) {
      ...
      $itemsScroll.scrollTop(scrollT > itemsScrollTop + itemsHeight ? scrollT - itemsHeight : liTop - liHeight < itemsScrollTop ? liTop - liHeight : itemsScrollTop);
    }
